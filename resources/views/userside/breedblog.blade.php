...
@foreach($posts as $post)
    <article class="blog_item">
        <div class="blog_item_img">
            <!-- Loop through each image for this post -->
...
<nav class="blog-pagination justify-content-center d-flex">
 ...
 {{-- <aside class="single_sidebar_widget post_category_widget">
    <h4 class="widget_title">Pet Category</h4>
    <ul class="list cat-list">
        @foreach($categories as $category)
            <li>
                <a href="" class="d-flex">
                    <p>{{ $category->name }}</p>
                    <p>({{ $category->posts_count }})</p> <!-- Display post count for each category -->
                </a>
            </li>
        @endforeach
    </ul>
</aside> 
 <aside class="single_sidebar_widget breed_widget">
    <h4 class="widget_title">Breeds in {{ $category->name }}</h4> <!-- Display category name -->
    <ul class="list breed-list">
        @foreach($breeds as $breed)
            <li>
                <a href="" class="d-flex">
                <p>{{ $breed->name }}</p> 
            </a><!-- Display breed name -->
            </li>
        @endforeach
    </ul>
</aside>  --}}
